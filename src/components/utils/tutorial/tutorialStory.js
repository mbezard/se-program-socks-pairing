...
export const STEP_RESPONSE = "STEP_RESPONSE"
...
export const tutorialSteps = [
    ...
    {type: STEP_NOTHING},
    {type: STEP_QUESTION, text: 16},
    {type: STEP_NOTHING},
    {type: STEP_WAIT, time: 1500},
    {type: STEP_RESPONSE, text: 9},
    {type: STEP_MULTIPLE_TEXT, texts: [17, 18]},
    {type: STEP_NOTHING},
    {type: STEP_ONE_TEXT, text: 19},
        ...
        switch (tutorialSteps[step].type) {
            case STEP_MULTIPLE_TEXT:
                if (progress < smallStep + tutorialSteps[step].texts.length) {
                    return {
                        type: STEP_MULTIPLE_TEXT,
                        text: tutorialTexts[tutorialSteps[step].texts[progress - smallStep]]
                    }
                }
            ...
            case STEP_NOTHING:
                ...
                break;
            case STEP_QUESTION:
                if (progress === smallStep) return {
                    type: STEP_QUESTION,
                    text: tutorialTexts[tutorialSteps[step].text],
                }
                smallStep += 1
                break;
            case STEP_RESPONSE:
                if (progress === smallStep) return {
                    type: STEP_RESPONSE,
                    text: tutorialTexts[tutorialSteps[step].text],
                    correctAnswerIndex: tutorialSteps[step].correctAnswerIndex
                }
                smallStep += 1
                break;
            default:
